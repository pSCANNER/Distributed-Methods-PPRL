...
public class BlockingJoin extends AbstractJoin {
	...
	public static final String MAX_CPU = "max-cpu";
	...
	public BlockingJoin(AbstractDataSource sourceA, AbstractDataSource sourceB,
		...
		intMaxCPU = Integer.parseInt(getProperty(MAX_CPU).toString().trim());
	...
	protected DataRow doJoinNext() throws IOException, RJException {
		...
		if (!initialized ) {
			...
			if(CPUInfo.testNumberOfCPUs()<intMaxCPU){
				intMaxCPU = CPUInfo.testNumberOfCPUs();
			}
			
			latch = new CountDownLatch(intMaxCPU);
			hashers = new HashingThread[intMaxCPU];
	...
	private void createThreadsIfNeeded() {
		if (threads == null) {
			...
			threads = new BlockingJoinThread[intMaxCPU];
